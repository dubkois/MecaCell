...
template <typename Cell> struct SpringConnection {
	...
	void updateCollisionParams() {
		...
		collision.k =
		    ...
		    collision.restLength;
		collision.c =
		    dampingFromRatio(COLLISION_DAMPING_RATIO,
		                     cells.first->getMass() + cells.second->getMass(), collision.k);
	...
	void update(double dt) {
		...
		updateCollisionParams();
		collision.applyForce(*(cells.first), *(cells.second), direction, dt);
		if (adhesionEnabled) {
